...
export function getFlags(client: ModbusRTU, unitIds: number[]): Promise<void> {
  return new Promise((resolve, reject) => {
    const requests = unitIds.map((id) => ({ id, register: 110 }));
    ...
    const processNext = () => {
      ...
      send(client, req.id, req.register, 1)
        .then((values) => {
          ...
          console.log('ID:', req.id, 'Flags', values[0]);
